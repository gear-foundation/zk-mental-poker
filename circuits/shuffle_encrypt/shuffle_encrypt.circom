...
template ShuffleEncrypt(numCards) {
    ...
    signal input permutation[numCards];     // permutation indices
    signal output isValid;                  // output
    signal output debugEncrypted[6][numCards];

    component encryptor = ShuffleEncryptTemplate(baseX, baseY, numCards, numBits);
    for (var i = 0; i < 3; i++) {
        encryptor.pk[i] <== pk[i];
    }
    for (var i = 0; i < numCards; i++) {
        encryptor.R[i] <== R[i];
        encryptor.permutation[i] <== permutation[i];
        for (var j = 0; j < 6; j++) {
    ...
    for (var i = 0; i < numCards; i++) {
    for (var j = 0; j < 6; j++) {
        debugEncrypted[j][i] <== encryptor.debugEncrypted[j][i];
    }
}
